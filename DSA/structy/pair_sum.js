// pair sum
// Write a function, pairSum, that takes in an array and a target sum as arguments. 
// The function should return an array containing a pair of indices whose elements 
// sum to the given target. The indices returned must be unique.
// Be sure to return the indices, not the elements themselves.
// There is guaranteed to be one such pair that sums to the target.

// const pairSum = (numbers, targetSum) => {
//  for(let i = 0; i < numbers.length; i++){
//     for(let j =1; j <numbers.length; j++){
//       if(numbers[i] + numbers[j] === targetSum){
//         console.log("numbers[i] + numbers[j]:", numbers[i], numbers[j] )
//          console.log("i, j:",i, j )
//         return [i, j]
//       }
//     }
//  }
// };

// const pairSum = (numbers, targetSum) => {
//   const prevNums = {}
//   for(i=0; i < numbers.length; i++){
//     const currentNum = numbers[i]
//     console.log("currentIndex", i)
//     console.log("currentNum", currentNum)
//     console.log("prevNums", prevNums)
//     const complement = targetSum - currentNum 
//     console.log("complement", complement)
//     console.log("prevNums", prevNums)
//     prevNums[currentNum] = i;
//     if(complement in prevNums){
//     console.log("prevNums", prevNums)
//      console.log("currentIndex", i)
//      console.log("prevNums[complement]",[prevNums[complement], i])
//       return [prevNums[complement], i]
//     }
//     prevNums[currentNum] = i;
//   }
// };

const pairSum = (numbers, targetSum) => {
  const prevNums = new Map();
  for(i=0; i <numbers.length; i++){
   const currentNum = numbers[i];
   const complement = targetSum - currentNum;
   if(prevNums.has(complement)){
    console.log("prevNums.get(complement)", prevNums.get(complement), i)
    return [prevNums.get(complement), i]
   }
   prevNums.set(currentNum, i)
  }
 
};

// n = length of array
// Time: O(n)
// Space: O(n)

pairSum([3, 2, 5, 4, 1], 8); // -> [0, 2]
// pairSum([4, 7, 9, 2, 5, 1], 5); // -> [0, 5]
// pairSum([4, 7, 9, 2, 5, 1], 3); // -> [3, 5]
// pairSum([1, 6, 7, 2], 13); // -> [1, 2]
// pairSum([9, 9], 18); // -> [0, 1]
// pairSum([6, 4, 2, 8 ], 12); // -> [1, 3]
// pairSum([5, 4, 1, 4], 8); // -> [1, 3]
// const numbers = [];
// for (let i = 0; i <= 30000; i += 1) {
//   numbers.push(i);
// }
// pairSum(numbers, 59999); // -> [ 29999, 30000 ] 

