...
// reverse() has only one purpose: reverse the order of the elements.
// Usage is arrayObject.reverse() and don't need parameters. Example:
...
// Here we start to learn about the second methods of sorting: sort().
// Its usage:

arrayObject.sort([compareFunction]);

// The parameter compareFunction is optional.
...
// if arr is a number array and the parameter is omitted,
// sort() will first convert the number into a string, and then sort the array.
// Example:
...
// This result may not be what we want.
// If you want to sort the numbers according to the size of their values,
// you must use a parameter, the parameter is a function. A classic example:
...
// Usually this is the result we want. (a, b)=> a - b is arrow function.
// The formal format is: function(a, b) { return a - b }.
// If you forget how to use the arrow function,
// please refer to:#22: Unlock new skills--Arrow function,spread operator and deconstruction
...
// If we need to sort by two or more conditions, we need to spend more energy
// to write a proper comparison function. The following example has two sorting
// conditions, first according to the classification of odd and even order,
// and then follow the numbers in ascending order.
...
// it's a number array. Your task is to sort the array according to the specified conditions,
...
/******** Task Solution 2 ********/
// function sortIt(arr) {
//   const cnt = arr.reduce((o, n) => ((o[n] = o[n] + 1 || 1), o), {});
//   return [...arr].sort((a, b) => cnt[a] - cnt[b] || b - a);
// }
/******** Task Solution 3 ********/
// const sortIt = (arr) => {
//   var res = [...arr];
