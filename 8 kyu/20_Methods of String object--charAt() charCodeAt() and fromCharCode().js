// This time we learn three string methods used to single character operation:
...
// In practical use, the charAt() method is usually replaced by str[index].
...
// charCodeAt() method can return the specified location of the character of the Unicode encoding.
// This return value is an integer between 0 and 65535.

// Method charCodeAt() and charAt() operation is similar, but the former is returned
// to the specified location of the character encoding,
...
// This is the reference information of unicode.
...
// If parameter index of charCodeAt() is omitted,
// its returned value will be the first character of stringObject.
// So the parameter can be omitted when the string length is 1;
// in other cases, you can't omit the parameter,
// unless you want to get the first character.
...
// fromCharCode() can accept one or more specified Unicode values,
// and then return a string.
// The method is a static method of String.
// It cannot be used as a method of the String object you have created.
// So its syntax should be String.fromCharCode() instead of stringObject.fromCharCode().
// See example:
...
// When fromCharCode() has only one parameter,
// it is very much like the reverse operation of charCodeAt().
// they are a pair of good partners.We often see them in the kata
// about the shift encryption.See an example:
...
// This is a simple example of the shift of encryption,
// encryption to the right(Unicode value becomes large) shift 1.
...
// Shift is only for uppercase and lowercase letters,
...
// Some trainer may have doubts about the three questions,
// the following is some explanation, I hope to be helpful to the trainer.

// for example, after you click "submit", you see these text on your screen:

// Top secret files: No. 3330
// Super agent eEB, on a dark night, stole the Apple core from National Museum,
...
// Super agent ZIUScE, on a dark night, stole the Lolli pop from National Museum,
// causing $100 million in damage.
// You only need to choose one of the text as the answer to the question.
// That is to say, any one of them is the correct answer.
