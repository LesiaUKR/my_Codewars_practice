...
// The two method is approximate.It applies a function against an accumulator
// and each value of the array to reduce it to a single value.
// The only difference is: the order of reduce() is from left to right,
// but reduceRight() is from right to left.

// For more information, please refer to:
Array.prototype.reduce();
Array.prototype.reduceRight();

// Before looking at some examples, we need to understand
// that the callback function used for the reduce()
// can use up to 4 parameters: previousValue, currentValue, index and array.
// In general, we use only two parameters: previousValue and currentValue.
// index and array are optional, and we use it when necessary.

// Here are some examples to help us understand the
// use of reduce() and reduceRight():

// The first is a classic example, we can use reduce()
// method to get the sum of the number array:
...
// In addition to the callback function, reduce() also has an
// optional parameter: initialvalue.In the example above,
// the initialvalue is omitted.Then the initialvalue is the first element,
// and then start the cumulative operation from second element.
...
// This example uses an empty string as the initialvalue,
// followed by these operation:
...
// The following is a complex example, which uses a callback
// function of the four parameters and the initialvalue:
...
[1, 2, 3, 4, 5][ //-- -> [1, 4, 9, 16, 25]
  // Then, using the reduce method.the two elements(ar[i] and arr2[i])
  // of the same index in turn are put into the initial value(an empty array).
...
// It should be noted that when reading the value of the first array,
// we used the ar[i](fourth parameters of the callback function).
...
// First, from left to right, the tail of an element and the
